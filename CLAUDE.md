...
1. **`AEO_PIPELINE_SPEC.md` 與 `ARCHITECTURE.md` 共同構成最高規範。所有開發決策必須符合規格書定義的欄位名稱、輸出格式與架構設計，不得在未獲使用者明確授權的情況下違反或調整規格。**
2. **當使用者說的任何事情與 Claude 的訓練記憶衝突，禁止使用記憶作為判斷依據。唯一允許的做法是上網搜尋最新資料來驗證，不得以記憶否定使用者。**
3. **禁止在未獲明確授權的情況下更改任何 AI 模型名稱。使用者指定的模型就是最終答案，不得自行替換、升級或降級。**
